...
def mutate_env_params(env_params : EnvParams, rng : chex.PRNGKey):
    ...
    return env_params.replace(  gravity = 9.8 + jax.random.uniform(keys[0], minval=-noise_scale, maxval=noise_scale),
                                masscart = 1.0 + jax.random.uniform(keys[1], minval=-noise_scale, maxval=noise_scale),
                                masspole = 0.1 + jax.random.uniform(keys[2], minval=-noise_scale, maxval=noise_scale),
                                total_mass = 1.0 + 0.1 + jax.random.uniform(keys[3], minval=-noise_scale, maxval=noise_scale),  # (masscart + masspole)
                                length = 0.5 + jax.random.uniform(keys[4], minval=-noise_scale, maxval=noise_scale),
                                polemass_length = 0.05 + jax.random.uniform(keys[5], minval=-noise_scale, maxval=noise_scale),  # (masspole * length)
                                force_mag = 10.0 + jax.random.uniform(keys[6], minval=-noise_scale, maxval=noise_scale),
                                tau = 0.02 + jax.random.uniform(keys[7], minval=-noise_scale, maxval=noise_scale))
...
def get_meta_learner_fn(
    ...
    def inner_policy_update_fn(inner_params : hk.Params, inner_opt_state : optax.OptState, rng_key : chex.PRNGKey, env_params : EnvParams):
        ...
        inner_loss, grads = jax.value_and_grad(  # compute gradient on a single trajectory.
            loss)(inner_params, rollout)
        grads = jax.lax.pmean(grads, axis_name="tasks")  # average gradients across tasks.
        grads = jax.lax.pmean(grads, axis_name="devices")  # average gradients across devices.
    ...
    def inner_loop_fn(meta_params : hk.Params, meta_opt_state : optax.OptState, rng_key : chex.PRNGKey, env_params):
        ...
        #     inner_params, inner_opt_state, inner_loss = inner_policy_update_fn(inner_params, inner_opt_state, rng_key, env_params)
        ...
        rollout = jax.vmap(sample_trajectory, in_axes=(None, 0, None))(meta_params, trajectory_keys, env_params)
        batch_loss = jax.vmap(inner_policy_loss_fn, in_axes=(None, 0))
        loss = jnp.mean(batch_loss(meta_params, rollout))

        return loss
    
    def meta_loss(meta_params, meta_opt_state, inner_loop_key, task_env_param):
            """Computes the meta-loss."""
            outer_loss = inner_loop_fn(meta_params, meta_opt_state, inner_loop_key, task_env_param)
            outer_loss = jnp.mean(outer_loss)
            return outer_loss
        
    def outer_fn(meta_params : hk.Params, meta_opt_state : optax.OptState, rng_key : chex.PRNGKey, task_env_param : EnvParams):
        ...
        # meta_loss_value, grads = jax.value_and_grad(meta_loss)(meta_params, meta_opt_state, inner_loop_key, task_env_param)
        # grads = jax.lax.pmean(grads, axis_name="tasks")  # average gradients across tasks.
        # grads = jax.lax.pmean(grads, axis_name="devices")  # average gradients across devices.
        ...
        new_meta_params, new_meta_opt_state, meta_loss_value = inner_policy_update_fn(meta_params, meta_opt_state, inner_loop_key, task_env_param)
    ...
    def meta_learner_fn(init_meta_params, init_meta_opt_state, rng_key : chex.PRNGKey):
        ...
        batched_outer_fn = jax.vmap(outer_fn, in_axes=(0, 0, 0, 0), axis_name="tasks")

        def iterate_fn(carry, rng_key):  # repeat many times to avoid going back to Python.
            meta_params, meta_opt_state, task_env_params = carry
            new_meta_params, new_meta_opt_state, meta_loss = batched_outer_fn(meta_params, meta_opt_state, jax.random.split(rng_key, num_tasks), task_env_params)
            return (new_meta_params, new_meta_opt_state, task_env_params), meta_loss
        
        (new_meta_params, new_meta_opt_state, task_env_params), meta_loss = jax.lax.scan(iterate_fn, (init_meta_params, init_meta_opt_state, task_env_params), jax.random.split(rng_key, iterations))
...
if __name__ == "__main__":
    ...
    meta_training_steps = 50
    print("Meta-learning...")

    key, train_key = jax.random.split(key)
    train_keys = jax.random.split(train_key, jax.local_device_count())
    reshape = lambda x: x.reshape((jax.local_device_count(), ) + x.shape[1:])
    train_keys = reshape(jnp.stack(train_keys)) 

    meta_params, meta_opt_state = broadcast_to_shape((meta_params, meta_opt_state), (jax.local_device_count(),num_tasks))

    for i in range(meta_training_steps):
        meta_params, meta_opt_state, meta_loss = meta_learn_fn(meta_params, meta_opt_state, train_keys)
        print("Meta Loss: ", jnp.mean(meta_loss))

    meta_params = extract_from_first_device(meta_params)
    key, eval_key = jax.random.split(key)
    eval_keys = jax.random.split(eval_key, 100)
    tot_reward = eval_fn(meta_params, eval_keys, init_env_params)[-1]
